    ...
    <div id="content" class="span10">
        <ul class="breadcrumb">
            <li>
                ...
                <a>Schema EA kodova</a>
            </li>
        </ul>
        <div class="row-fluid sortable">
            <div class="box span12 responsive-tabela">
                <div class="box-header" data-original-title id="height">
                    <h2><i class="halflings-icon white user"></i><span class="break"></span>Schema EA kodova</h2>
                </div>
                <div class="box-content">
                    <table class="table table-striped table-bordered bootstrap-datatable datatable">
                        ...
                        <tr>
                            <th></th>
                            <c:if test="${not empty eaCodes}">
                                <c:forEach var="eaCode" items="${eaCodes}">
                                    <th>${eaCode.codeNumber}</th>
                                </c:forEach>
                            </c:if>
                        </tr>
                        </thead>
                        ...
                        <c:if test="${not empty appUsers}">
                            <c:forEach var="user" items="${appUsers}">
                                <tr>
                                    <td>${user.name}</td>
                                    <c:forEach var="code" items="${eaCodes}">
                                        <td style="text-align: center; vertical-align: middle">
                                            <i id="${code.id} + ${user.id}"></i>
                                        </td>
                                        <c:forEach var="ea" items="${user.eaCodes}">
